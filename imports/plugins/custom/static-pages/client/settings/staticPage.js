...
Template.genericForm.events({
  "submit form": event => {
    ...
    const pageTitle = field.pageTitle.value;
    const pageURLAddress = field.pageUrl.value;
    const pageContent = editor.value();
    let createdAt = new Date();
    const updatedAt = new Date();
    if (pageTitle === "" || pageURLAddress === "" || pageContent === "") {
    ...
    if (
      ...
      const checkPageDetails = StaticPages.findOne({ $or: [{ pageTitle: pageTitle }, { pageURLAddress: pageURLAddress }] });
      if (checkPageDetails) {
        return Alerts.toast("The page name/address exists already", "error");
      }
      Meteor.call("createStaticPage", pageTitle, pageURLAddress, pageContent, createdAt, function (err) {
        if (err) {
      ...
      if (pageDetails.length > 0) {
        ...
        Meteor.call("updateStaticPage", _id, pageTitle, pageURLAddress, pageContent, createdAt, updatedAt, function (err) {
          if (err) {
            Alerts.toast(err.message, "error");
          } else {
...
Template.manageStaticPage.events({
  ...
  "click .editStaticPage"(event) {
    ...
    $(".static-page")
      .find("#page-title")
      .val(pageDetails[0].pageTitle);
    $(".static-page")
      .find("#page-url")
      .val(pageDetails[0].pageURLAddress);
    $(".static-page")
